...
% figure is drawn
tmpfilename = 'figseird_liberia_v4';
...
tmpi=find(~isnan(x.Cases_Liberia) & x.Day>=92 & x.Day<=162);
[p,s]=polyfit(x.Day(tmpi)-x.Day(tmpi(end)),log(x.Cases_Liberia(tmpi)),1)
lambda_target = p(1);  % Exponential growth rate as fit to the data
estats.p=p;
estats.s=s;
estats.time=x.Day(tmpi(end:-1:1));
estats.cases_g=x.Cases_Liberia(tmpi(end:-1:1));
tmpi=find(~isnan(x.Cases_Liberia) & x.Day>=92);
estats.time_all=x.Day(tmpi);
estats.cases_all=x.Cases_Liberia(tmpi);
...
for i=1:length(tmpcasebeta);
  ...
  info.N=4.3*10^6;  % Irrelevant at early times, as long as sufficiently high
  ...
  if (i==1) % Background line all looks the same
    tmph=plot(92+curt(tshift)-curt(tshift(1)),cumcases(tshift),'k-');
    set(tmph,'linewidth',3);
    hold on
  end
  tmph=plot(92+curt(tshift+tmptshift*(i-1):3*tmptshift:end)-curt(tshift(1)),cumcases(tshift+tmptshift*(i-1):3*tmptshift:end),'ko');
...
for i=1:length(tmpcasebeta);
  ...
  cumcases = sum(y(:,[8:11])');
  % Case counts
  tmph=semilogy(92+curt(tshift)-curt(tshift(1)),cumcases(tshift),'k-');
  set(tmph,'linewidth',3);
